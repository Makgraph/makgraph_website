...
const MainAddProduct = () => {
  ...
  const [sizes, setSizes] = useState([]); // Champ pour les tailles
  const [colors, setColors] = useState(""); // Champ pour les couleurs
  ...
  useEffect(() => {
    if (!loading && !error) {
      ...
      setCountInStock(0);
      setDescription("");
      setSizes([]); // Réinitialiser le champ tailles
      setColors(""); // Réinitialiser le champ couleurs
    }
  }, [loading, error]);
  ...
  const submitHandler = (e) => {
    ...
    dispatch(
      createProduct({
        ...
        description,
        sizes,
        colors: colors.split(",").map((color) => color.trim()), // Convertir en tableau
      })
    );
  };
