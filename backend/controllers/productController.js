...
//   // Compter les produits correspondant au mot-clé
//   const count = await Product.countDocuments(keyword);

//   // Récupérer les produits avec le mot-clé et la pagination
...
//     count, // Nombre total de produits correspondant au mot-clé
//   });
// });
// @desc Fetch all products
// @route GET /api/products
// @access Public
const getAllProducts = asyncHandler(async (req, res) => {
  const pageSize = 4; // Nombre de produits par page
  const page = Number(req.query.pageNumber) || 1; // Récupérer le numéro de page
  ...
  // Filtrer par `sizes` et `colors` si présents dans les requêtes
  const filters = {
    ...keyword,
  ...
  // Compter les produits correspondant aux filtres
  const count = await Product.countDocuments(filters);

  // Récupérer les produits avec les filtres et la pagination
  ...
  res.json({
    ...
    page,
    pages: Math.ceil(count / pageSize),
    count, // Nombre total de produits correspondant aux filtres
  });
});

// @desc Admin get all products with search and pagination
...
const createProduct = asyncHandler(async (req, res) => {
  ...
  const { name, price, description, image, countInStock, sizes, colors } =
    req.body;
  const productExist = await Product.findOne({ name });
  ...
  if (productExist) {
    ...
    const product = new Product({
      ...
      image,
      countInStock,
      sizes: sizes || [], // Initialisation avec un tableau vide si non défini
      colors: colors || [], // Initialisation avec un tableau vide si non défini
    });
...
const editProduct = asyncHandler(async (req, res) => {
  ...
  if (product) {
    ...
    product.image = image || product.image;
    product.countInStock = countInStock || product.countInStock;
    product.sizes = sizes || product.sizes; // Ajouter ici
    product.colors = colors || product.colors; // Ajouter ici
